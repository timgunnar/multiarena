...
```bash
multiarena              # 启动 Web 界面（推荐，无需参数）
multiarena web          # 显式 Web 模式
multiarena web 8080     # 自定义端口
multiarena terminal     # 终端模式（CLI）
```
...
- **任务卡片**：常用工作流一键模板（写文案、分析数据、出主意、审文档）
- **广播模式**：多面板流式输出，对比模型回答
- **团队审议**：轮次管线可视化 + 进度跟踪
- **权限弹窗**：模态对话框 y/n/a/d 确认工具调用
- **会话侧栏**：新建/切换/删除会话
- **中英文切换**：顶部 EN/中 语言切换按钮
- **设置页**：随时添加/删除模型、更新 API Key
- **设置向导**：首次使用引导配置，无需编辑配置文件
- **配置持久化**：模型保存到 `.multiarenarc`，与 CLI 共享
