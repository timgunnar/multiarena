...
[team] > 帮我写精品咖啡烘焙商「豆仓」的品牌故事，放在官网 About 页面▊
```
...
│ 1. minimax（起草）                                                   │
│ 2. deepseek（修订） — 5 处修改                                       │
│      我们从全球采购生豆 → 从埃塞俄比亚高原到哥伦比亚云雾林，            │
│      每一批豆子都可追溯到具体农场                                      │
│      经验丰富的烘焙师 → 能在二爆瞬间听出豆子情绪的烘焙师                │
│ 3. minimax（终审） — 无修改                                          │
│                                                                    │
│ ── 最终文档 ──                                                      │
│ # 豆仓 — 每一颗豆子都有自己的故事                                     │
...
```
按 Tab → [team:minimax] > 你觉得 deepseek 的修订抓住了烘焙工艺这个角度吗？
```

**继续修改：** 在团队总览输入新要求，启动新一轮审议。团队能看到完整的上轮产出并针对性修改：

```
[team] > 增加烘焙过程的感官细节——豆子爆裂的噼啪声、第一爆的焦香。这段文案用于包装标签，控制在 80 字以内
```
