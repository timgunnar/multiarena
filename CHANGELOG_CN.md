...
- **审议版本历史** — 每轮审议向模型展示所有历史版本的完整文档（带轮次编号和作者），模型可自由回退到任意早期版本
- **会话持久化修复** — teamMessages/token 用量/静音状态/输入历史 全面保存恢复
- **消息历史可见** — 用户消息在对话流中可见（`> 灰色前缀`），助手回复不再因新消息清空 buffer 而消失
- **对抗强度修复** — 2 模型 high 模式反向循环修正、-a 标志不再泄露到模型上下文、广播模式 `/team -a high` 快捷入口
- **测试基础设施** — 新增 Harness 类实现程序化 E2E 测试（广播/审议/持久化/清理），28 个新 E2E 测试，368 测试全过
