...
- **会话持久化补全** — teamMessages/token 用量/静音状态/输入历史全面保存恢复。
...
- 新增 120+ E2E 测试（广播 7、审议 11、持久化 7、清理 4、权限 6、合并 5、团队聊天 5、App 分支 35、安装 5）
- 服务器测试 12 个（HTTP + SSE）
- 全部 42 文件、440 测试通过
...
- **审议版本历史** — 每轮审议向模型展示所有历史版本（带轮次编号和作者），模型可回退到任意早期版本
- **审议修订保护** — 中间轮次允许模型间自由互改，终审把关；follow-up 时区分改写和重写指令
- **会话持久化补全** — `teamMessages`、token 用量、静音状态、输入历史全面保存恢复
- **消息历史可见** — 用户消息在对话流中可见，助手回复跨轮持久
- **对抗强度修复** — 2 模型 high 模式反向循环修正、`-a` 标志不泄露到模型上下文、广播模式 `/team -a high` 入口

### 测试

- 新增 112 个 E2E 测试（广播 7、审议 11、持久化 7、清理 4、权限 6、合并 5、团队聊天 5、App 分支 35、安装/卸载 5）
- 全部 40 文件、424 测试通过

## v0.1.7

- **README/CHANGELOG 中英文结构调整** — 默认为英文，中文提供 `_CN` 版本
...
- 新增 18 个测试用例（权限交互 8 个、turn 层 4 个、InputBar 3 个、集成测试 3 个）
- 全部 31 个测试文件、330 个测试用例通过
